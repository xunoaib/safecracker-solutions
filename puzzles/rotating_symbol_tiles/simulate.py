    ...
    if show_lines:
        for r in range(ROWS):
            for c in range(COLS):
                ...
                # # Brightness scales with value: 64 (dim) to 255 (bright)
                # brightness = int(64 + (val / 24.0) * (255 - 64))
                # cv2.line(grid_img, start, end, (brightness, ) * 3, 2)

                brightness = int(64 + (val / 24.0) * (255 - 64))

                cv2.line(grid_img, start, end, (0, brightness, brightness), 2)
